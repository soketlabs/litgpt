...
def convert_lit_checkpoint(checkpoint_dir: Path, output_dir: Path) -> None:
    ...
    with incremental_save(output_path) as saver:
        ...
        # print("\n==== LitGPT checkpoint (before mapping) ====")
        # print(f"len(lit_weights) = {len(lit_weights)}")
        # for k, v in lit_weights.items():
        #     if isinstance(v, NotYetLoadedTensor):
        #         # lazy tensor: shape is available without loading
        #         print(f"{k:5s} {tuple(v.shape)} (lazy)")
        #     else:
        #         print(f"{k:5s} {tuple(v.shape)}")
        check_conversion_supported(lit_weights)
        copy_fn(sd, lit_weights, saver=saver)
        # print("\n==== HuggingFace state_dict (after mapping) ====")
        # length = len(sd.items())
        # iter = 0
        # for item in sd.items():
        #     # if iter==0:
        #     print(f"{item[0]} after mapping")
        #     # break
