...
# # BASE_MODEL_DIR = Path("/projects/data/teams/tts_team/agri_training/local_gemma_3_27b")
# # HF_REFERENCE_MODEL = Path("/projects/data/teams/tts_team/agri_training/local_gemma_3_27b")
# # OUTPUT_DIR = Path("/projects/data/teams/tts_team/agri_training/finetuned_checkpoints_parquet/gemma3-27b-it-lora-2700/step-005625/lora_2700_2ndCrash/step-008375/hf_converted")
...
# # ============= CONFIGURATION =============
# LITGPT_LORA_CHECKPOINT = Path("/projects/data/teams/tts_team/agri_training/finetuned_checkpoints_parquet/gemma3-27b-it-lora-2700/step-005625/lora_2700_2ndCrash/step-008000")
# HF_REFERENCE_MODEL = Path("/projects/data/teams/tts_team/agri_training/checkpoints/google/gemma-3-27b-it/google/gemma-3-27b-it/")
# OUTPUT_DIR = Path("/projects/data/teams/tts_team/agri_training/finetuned_checkpoints_parquet/gemma3-27b-it-lora-2700/step-005625/lora_2700_2ndCrash/step-008000/hf_converted")
...
# Make sure LitGPT is importable
sys.path.insert(0, "/home/aditya.borate/litgpt")
...
HF_REFERENCE_MODEL = BASE_MODEL_DIR

OUTPUT_DIR = Path(
    "/projects/data/teams/tts_team/agri_training/finetuned_checkpoints_parquet/"
    "gemma3-27b-it-lora-2700/step-005625/lora_2700_2ndCrash/step-008375/hf_converted"
)

# -------- Model constants (Gemma-3-27B) --------
LITGPT_VOCAB_SIZE = 262144
HF_VOCAB_SIZE = 262208
HIDDEN_SIZE = 5376
NUM_LAYERS = 62
...
def pad_embedding(weight, target_vocab_size):
    """Pad embedding or lm_head weights to target vocab size."""
    current_vocab, hidden = weight.shape
    if current_vocab >= target_vocab_size:
        return weight

    pad_rows = target_vocab_size - current_vocab
    padding = torch.zeros((pad_rows, hidden), dtype=weight.dtype)
    return torch.cat([weight, padding], dim=0)
...
def convert_litgpt_to_hf(merged_model_path: Path):
    ...
    # --------------------------------------------------------
    # 1b. LM HEAD (PAD SEPARATELY, tie_word_embeddings = False)
    # --------------------------------------------------------
    if "lm_head.weight" in lit_sd:
        lm_head_weight = lit_sd["lm_head.weight"]
    elif "transformer.lm_head.weight" in lit_sd:
        lm_head_weight = lit_sd["transformer.lm_head.weight"]
    else:
        raise KeyError("❌ lm_head.weight not found in LitGPT checkpoint")

    padded_lm_head = pad_embedding(lm_head_weight, HF_VOCAB_SIZE)
    hf_sd["language_model.lm_head.weight"] = padded_lm_head

    print(f"  lm_head:     {list(lm_head_weight.shape)} -> {list(padded_lm_head.shape)}")
    ...
    for layer_idx in range(NUM_LAYERS):
        ...
        # Input layernorm
        hf_sd[f"{hf_prefix}.input_layernorm.weight"] = lit_sd[
            f"{lit_prefix}.norm_1.weight"
        ]
        ...
        # Output projection
        hf_sd[f"{hf_prefix}.self_attn.o_proj.weight"] = lit_sd[
            f"{lit_prefix}.attn.proj.weight"
        ]

        # Q/K norms (Gemma specific)
        if f"{lit_prefix}.attn.norm_q.weight" in lit_sd:
            hf_sd[f"{hf_prefix}.self_attn.q_norm.weight"] = lit_sd[
                f"{lit_prefix}.attn.norm_q.weight"
            ]
        if f"{lit_prefix}.attn.norm_k.weight" in lit_sd:
            hf_sd[f"{hf_prefix}.self_attn.k_norm.weight"] = lit_sd[
                f"{lit_prefix}.attn.norm_k.weight"
            ]

        # Post-attention layernorm
        if f"{lit_prefix}.post_attention_norm.weight" in lit_sd:
            hf_sd[f"{hf_prefix}.post_attention_layernorm.weight"] = lit_sd[
                f"{lit_prefix}.post_attention_norm.weight"
            ]

        # Pre-FFN layernorm
        if f"{lit_prefix}.norm_2.weight" in lit_sd:
            hf_sd[f"{hf_prefix}.pre_feedforward_layernorm.weight"] = lit_sd[
                f"{lit_prefix}.norm_2.weight"
            ]

        # MLP weights
        hf_sd[f"{hf_prefix}.mlp.gate_proj.weight"] = lit_sd[
            f"{lit_prefix}.mlp.fc_1.weight"
        ]
        hf_sd[f"{hf_prefix}.mlp.up_proj.weight"] = lit_sd[
            f"{lit_prefix}.mlp.fc_2.weight"
        ]
        hf_sd[f"{hf_prefix}.mlp.down_proj.weight"] = lit_sd[
            f"{lit_prefix}.mlp.proj.weight"
        ]

        # Post-MLP norm (optional)
        if f"{lit_prefix}.post_mlp_norm.weight" in lit_sd:
            hf_sd[f"{hf_prefix}.post_feedforward_layernorm.weight"] = lit_sd[
                f"{lit_prefix}.post_mlp_norm.weight"
            ]

        if (layer_idx + 1) % 10 == 0:
            print(f"  ✔ Converted layer {layer_idx + 1}/{NUM_LAYERS}")

    print(f"\n✔ Converted {len(hf_sd)} language model tensors")
...
def copy_vision_tower():
    ...
    print(f"Found {len(vision_keys)} vision-related weights")
    ...
    print(f"✔ Copied {len(vision_weights)} vision tensors")
    return vision_weights


# ============================================================
# STEP 4: CREATE HF CONFIG
# ============================================================
def create_hf_config():
    """Create HuggingFace config.json."""
    config = {
        ...
        "text_config": {
            ...
            "sliding_window": 1024,
            "sliding_window_pattern": 6,
            "tie_word_embeddings": False,   # ✅ IMPORTANT CHANGE
            "use_cache": True,
            "vocab_size": HF_VOCAB_SIZE,
        },
        "torch_dtype": "bfloat16",
...
# ============================================================
# STEP 5: SAVE SAFETENSORS SHARDS
# ============================================================
def save_as_safetensors(all_weights: Dict[str, torch.Tensor], output_dir: Path):
    """Save weights as sharded safetensors files with index."""
    print("\n" + "=" * 80)
    print("STEP 5: SAVING AS SAFETENSORS")
    print("=" * 80)
    ...
    print(f"\n✔ Saved {total_shards} shards with {len(weight_map)} total tensors")
...
def copy_tokenizer_files(output_dir: Path):
    print("\n" + "=" * 80)
    print("STEP 6: COPYING TOKENIZER FILES")
    print("=" * 80)
...
def main():
    print("=" * 80)
    print("LITGPT → HUGGINGFACE CONVERSION (EXPLICIT lm_head PADDING)")
    print("=" * 80)
